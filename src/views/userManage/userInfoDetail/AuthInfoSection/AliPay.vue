      ...
      <div v-else
        ...
        <cc-panel title='支付宝基本信息'
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="8">
              <div class='simple-text'>支付宝注册日期：{{userInfo.register_time}}</div>
              <div class='simple-text'>芝麻分：TODO</div>
            </el-col>
          </el-row>
        </cc-panel>
        ...
        <cc-panel title='资产状况信息'>
          <el-row :gutter="20">
            <el-col :span="8">
              <div class='simple-text'>账户余额：{{formatMoney(wealth.yue)}}元</div>
              <div class='simple-text'>余额宝金额：{{formatMoney(wealth.yeb)}}元</div>
              <div class='simple-text'>招财宝金额：{{formatMoney(wealth.zcb)}}元</div>
              <div class='simple-text'>存金宝金额：{{formatMoney(wealth.cjb)}}元</div>
            </el-col>
            <el-col :span="8">
              <div class='simple-text'>花呗授信额度：{{formatMoney(wealth.huabai_limit)}}元</div>
              <div class='simple-text'>当前可用额度：{{formatMoney(wealth.huabai_balance)}}元</div>
              <div class='simple-text'>当前是否有逾期：{{wealth.huabei_overdue ? '逾期' : '未逾期'}}</div>
              <div class='simple-text'>花呗逾期金额：{{formatMoney(wealth.huabei_overdue_amount)}}元</div>
            </el-col>
            <el-col :span="8">
              <div class='simple-text'>是否已开通借呗 ：{{alipayJiebei.new_able ? '未开通' : '已开通'}}</div>
              <div class='simple-text'>借呗授信额度：{{formatMoney(alipayJiebei.credit_amt)}}元</div>
              <div class='simple-text'>当前可用额度：{{formatMoney(alipayJiebei.loanable_amt)}}元</div>
              <div class='simple-text'>日利率：{{alipayJiebei.risk_int_by_thousand}}
                <span style='color: #999999;padding-left: 15px;'>(如：0.5 即表示1000元一天利息0.5元)</span>
              </div>
